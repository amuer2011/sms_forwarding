...
- 支持长短信自动合并（30秒超时）
- 支持管理员短信远程发送短信和重启设备
- 支持模组网络注册自动恢复，兼容本地卡和国际漫游 SIM/eSIM
...
固件启动后优先让 ML307R 自动选网，并同时将 `CEREG=1`（本地注册）和
`CEREG=5`（漫游注册）视为注册成功。若自动注册持续失败，固件会按当前 SIM
的 ICCID 读取上次确认成功的运营商并尝试恢复；没有可用记录或恢复失败时，
再执行一次运营商扫描，并依次尝试有限数量的候选网络。

恢复成功后会更新当前 SIM 的成功运营商记录。不同 SIM 的记录相互隔离，换卡后
不会沿用上一张卡的记录。通过候选网络恢复成功时，本次启动周期保持当前连接；
设备下次冷启动仍会先从自动选网开始。

网络恢复由主循环分步执行，不会长时间阻塞 ESP32-C3。恢复期间主页、配置、日志
和 WiFi 功能仍可使用；需要直接访问模组串口的网页操作会返回 HTTP 429，待恢复
流程释放串口后即可重试。运营商扫描可能持续数分钟，但每轮只执行一次，全部候选
失败后会回到自动模式并退避一段时间再重试。
