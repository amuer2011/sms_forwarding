...
CPIN READY / 读取 ICCID
    │
    ▼
AT+COPS=0 自动选网，等待 120 秒
    │失败
    ▼
按 ICCID 读取 last-good PLMN，最长尝试 180 秒
    │无缓存、failCount>=3 或尝试失败
    ▼
AT+COPS=? 单次流式扫描，最长 180 秒
    │
    ▼
最多 4 个候选，每个最长 60 秒
    │全部失败
    ▼
AT+COPS=0，退避 10 分钟后重新开始
```

候选按 `available -> current -> forbidden -> unknown` 排序，`forbidden` 只降低优先级，不会被删除。手动注册成功后当前启动周期保持手动 PLMN；下次冷启动仍从自动选网开始。固件不写 SIM 的 `EF_FPLMN`。

恢复事务持有 `Serial1` 所有权，每次 `loop()` 最多读取 64 字节。普通 HTTP 页面继续响应；需要访问模组的接口在恢复期间返回 HTTP 429。`+CMT` 和后续 PDU 即使夹在 AT 响应中，也会转交短信解析器。
...
- 网络扫描解析器：固定 12 个扫描结果、4 个候选，不保存完整 `COPS=?` 响应
...
- **模组基础 AT 无响应**：初始化阶段持续重试，LED 闪烁指示
- **自动注册失败**：last-good → 单次扫描 → 最多 4 个候选 → 10 分钟退避
- **运营商被标记 forbidden**：保留为低优先级候选，允许实际手动尝试
